...
__성별 별로 신뢰구간 계산하기__
```{python}
f_conv1 = f_bar - z_0025 * f_std/np.sqrt(f_n)
f_conv2 = f_bar + z_0025 * f_std/np.sqrt(f_n)
m_conv1 = m_bar - z_0025 * f_std/np.sqrt(m_n)
m_conv2 = m_bar + z_0025 * f_std/np.sqrt(m_n)
```
...
```{python}
sns.barplot(data = sex_income, x = 'sex', y = 'mean_income', hue = "sex")
plt.vlines(x = "female", ymin = f_conv1, ymax = f_conv2, colors = "black")
plt.vlines(x = "male", ymin = m_conv1, ymax = m_conv2, colors = "black")
plt.show()
plt.clf()
```
